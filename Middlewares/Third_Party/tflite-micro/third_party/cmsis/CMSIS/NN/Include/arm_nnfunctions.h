...
/**
   ...
   * - Legacy functions supporting ARM's internal symmetric quantization(8
   bits).
   * - Functions that support TensorFlow Lite framework with symmetric
   quantization(8 bits).
   *
   * The legacy functions can be identified with their suffix of _q7 or _q15 and
   are no new development is done there.
   * The article in [2] describes in detail how to run a network using the
   legacy functions.
   *
   * The functions supporting TensorFlow Lite framework is identified by the _s8
   suffix and can be invoked from TFL
   * micro. The functions are bit exact to TensorFlow Lite. Refer to the
   TensorFlow's documentation in [3] on how to run
   ...
   * - ARM_MATH_DSP:
   *
   * Define macro ARM_MATH_DSP, If the silicon supports DSP instructions(DSP
   extension).
   *
   * - ARM_MATH_MVEI:
   *
   * Define macro ARM_MATH_MVEI, If the silicon supports M-Profile Vector
   Extension.

   * - ARM_MATH_AUTOVECTORIZE
   *  Used in conjucture with ARM_MATH_MVEI to let the compiler auto vectorize
   for the functions that uses inline
   ...
   * Define macro ARM_MATH_BIG_ENDIAN to build the library for big endian
   targets. This is supported only for the legacy
   * functions i.e, functions targetted at TensorFlow Lite do not support big
   endianness. By default library builds for
 ...
 *
 * Collection of convolution, depthwise convolution functions and their
 * variants.
 *
...
/**
 * @brief s8 convolution layer wrapper function with the main purpose to call
 the optimal kernel available in cmsis-nn
 *        to perform the convolution.
 *
 * @param[in, out] ctx            Function context that contains the additional
 buffer if required by the function. arm_convolve_wrapper_s8_get_buffer_size
 will return the buffer_size if required
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 dilations, pads,...).
 *                                Range of conv_params->input_offset  : [-127,
 128]
 *                                Range of conv_params->output_offset : [-128,
 127]
 * @param[in]      quant_params   Per-channel quantization info.
 *                                It contains the multiplier and shift values to
 be applied to each output channel
 ...
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, HK,
 WK, C_IN] where HK and WK are the
 ...
 *                  <code>ARM_MATH_SIZE_MISMATCH</code> if argument constraints
 fail. or,
 *                  <code>ARM_MATH_SUCCESS</code> on successful completion.
 *
 */
arm_status arm_convolve_wrapper_s8(
 ...
 *
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 * dilations, pads,...). Range of conv_params->input_offset  : [-127, 128] Range
 * of conv_params->output_offset : [-128, 127]
 * @param[in]      input_dims     Input (activation) dimensions. Format: [N, H,
 * W, C_IN]
 * @param[in]      filter_dims    Filter dimensions. Format: [C_OUT, HK, WK,
 * C_IN] where HK and WK are the spatial filter dimensions
 * @param[in]      output_dims    Output tensor dimensions. Format: [N, H, W,
 * C_OUT]
 *
...
/**
 * @brief s16 convolution layer wrapper function with the main purpose to call
 the optimal kernel available in cmsis-nn
 *        to perform the convolution.
 *
 * @param[in, out] ctx            Function context that contains the additional
 ...
 *
 */
arm_status arm_convolve_wrapper_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q15_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
    const cmsis_nn_dims *bias_dims, const int64_t *bias_data,
    const cmsis_nn_dims *output_dims, q15_t *output_data);

/**
 * @brief Get the required buffer size for arm_convolve_wrapper_s16
 *
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 * dilations, pads,...). conv_params->input_offset  : Not used
 *                                conv_params->output_offset : Not used
 * @param[in]      input_dims     Input (activation) dimensions. Format: [N, H,
 * W, C_IN]
 * @param[in]      filter_dims    Filter dimensions. Format: [C_OUT, HK, WK,
 ...
 * @param[in, out] ctx            Function context that contains the additional
 buffer if required by the function. arm_convolve_s8_get_buffer_size will return
 the buffer_size if required
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 dilations, pads,...).
 *                                Range of conv_params->input_offset  : [-127,
 128]
 ...
 *    2. q7 is used as data type eventhough it is s8 data. It is done so to be
 consistent with existing APIs.
 *    3. Additional memory is required for optimization. Refer to argument 'ctx'
 for details.
 *
 */
 ...
 * Format: [N, H, W, C_IN]
 * @param[in]       filter_dims           Filter tensor dimensions. Format:
 * [C_OUT, HK, WK, C_IN] where HK and WK are the spatial filter dimensions
 * @return          The function returns  required buffer size(bytes)
 *
 */
 ...
 * @param[in, out] ctx            Function context that contains the additional
 buffer if required by the function. arm_convolve_s16_get_buffer_size will
 return the buffer_size if required
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 dilations, pads,...).
 *                                conv_params->input_offset  : Not used
 *                                conv_params->output_offset : Not used
 ...
 * @details
 *    1. Supported framework: TensorFlow Lite micro
 *    2. q7/q15 is used as data type eventhough it is s8/s16 data. It is done so
 to be consistent with existing APIs.
 *    3. Additional memory is required for optimization. Refer to argument 'ctx'
 for details.
 *
 */
arm_status arm_convolve_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q15_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
 ...
 * @param[in, out] ctx            Function context that contains the additional
 buffer if required by the function. arm_convolve_fast_s16_get_buffer_size will
 return the buffer_size if required
 * @param[in]      conv_params    Convolution parameters (e.g. strides,
 dilations, pads,...).
 *                                conv_params->input_offset  : Not used
 ...
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, HK,
 WK, C_IN] where HK and WK are the
 *                                spatial filter dimensions. (filter_dims->w *
 filter_dims->h * input_dims->c) must not exceed 512
 * @param[in]      filter_data    Filter data pointer. Data type: int8
 * @param[in]      bias_dims      Bias tensor dimensions. Format: [C_OUT]
 * @param[in]      bias_data      Optional bias data pointer. Data type: int64
 * @param[in]      output_dims    Output tensor dimensions. Format: [N, H, W,
 ...
 *    3. Additional memory is required for optimization. Refer to argument 'ctx'
 for details.
 *    4. Implementation supports kernel volumes (filter width * filter height *
 input channels) < 512.
 *
 */

arm_status arm_convolve_fast_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q15_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
 ...
 * @param[in,out]   bufferB      pointer to buffer space for output
 * @return     The function returns either
 *                          <code>ARM_MATH_SIZE_MISMATCH</code> if argument
 * constraints fail. or, <code>ARM_MATH_SUCCESS</code> on successful completion.
 *
 ...
 * @param[in]      input_data     Input (activation) data pointer. Data type:
 int8
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, 1,
 1, C_IN]
 * @param[in]      filter_data    Filter data pointer. Data type: int8
 * @param[in]      bias_dims      Bias tensor dimensions. Format: [C_OUT]
 * @param[in]      bias_data      Optional bias data pointer. Data type: int32
 ...
 *
 */
arm_status arm_convolve_1x1_s8_fast(
    const cmsis_nn_context *ctx, const cmsis_nn_conv_params *conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q7_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
 ...
 * @param[in]      input_data     Input (activation) data pointer. Data type:
 int8
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [C_OUT, 1,
 WK, C_IN] where WK is the horizontal
 ...
 * Format: [N, H, W, C_IN]
 * @param[in]       filter_dims           Filter tensor dimensions. Format:
 * [C_OUT, 1, WK, C_IN] where WK is the horizontal spatial filter dimension
 * @return          The function returns  required buffer size(bytes)
 *
 */
...
/**
 * @brief Wrapper function to pick the right optimized s8 depthwise convolution
 * function
 *
 * @param[in, out] ctx            Function context (e.g. temporary buffer).
 * Check the function definition file to see if an additional buffer is
 * required. Optional function {API}_get_buffer_size() provides the buffer size
 * if required.
 * @param[in]      dw_conv_params Depthwise convolution parameters (e.g.
 * strides, dilations, pads,...) dw_conv_params->dilation is not used. Range of
 * dw_conv_params->input_offset : [-127, 128] Range of
 * dw_conv_params->output_offset : [-128, 127]
 * @param[in]      quant_params   Per-channel quantization info.
 *                               It contains the multiplier and shift values to
 * be applied to each output channel
 * @param[in]      input_dims     Input (activation) tensor dimensions. Format:
 * [H, W, C_IN] Batch argument N is not used and assumed to be 1.
 * @param[in]      input_data     Input (activation) data pointer. Data type:
 * int8
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [1, H, W,
 * C_OUT]
 * @param[in]      filter_data    Filter data pointer. Data type: int8
 * @param[in]      bias_dims      Bias tensor dimensions. Format: [C_OUT]
 * @param[in]      bias_data      Bias data pointer. Data type: int32
 * @param[in]      output_dims    Output tensor dimensions. Format: [1, H, W,
 * C_OUT]
 ...
 *    - q7 is used as data type eventhough it is s8 data. It is done so to be
 * consistent with existing APIs.
 *    - Check details of arm_depthwise_conv_s8_opt() for potential data that can
 * be accessed outside of the boundary.
 */
arm_status arm_depthwise_conv_wrapper_s8(
...
/**
 * @brief Get size of additional buffer required by
 ...
 * @param[in]      output_dims    Output tensor dimensions. Format: [1, H, W,
 * C_OUT]
 * @return                        Size of additional memory required for
 * optimizations in bytes.
 *
 */
...
/**
 * @brief Basic s8 depthwise convolution function that doesn't have any
 * constraints on the input dimensions.
 *
 * @param[in, out] ctx            Function context (e.g. temporary buffer).
 * Check the function definition file to see if an additional buffer is
 * required. Optional function {API}_get_buffer_size() provides the buffer size
 * if an additional buffer is required. exists if additional memory is.
 * @param[in]      dw_conv_params Depthwise convolution parameters (e.g.
 * strides, dilations, pads,...) dw_conv_params->dilation is not used. Range of
 * dw_conv_params->input_offset : [-127, 128] Range of
 * dw_conv_params->input_offset : [-128, 127]
 ...
 * be applied to each output channel
 * @param[in]      input_dims     Input (activation) tensor dimensions. Format:
 * [1, H, W, C_IN] Batch argument N is not used.
 * @param[in]      input_data     Input (activation) data pointer. Data type:
 * int8
 * @param[in]      filter_dims    Filter tensor dimensions. Format: [1, H, W,
 * C_OUT]
 ...
 * consistent with existing APIs.
 */
arm_status arm_depthwise_conv_s8(
    const cmsis_nn_context *ctx, const cmsis_nn_dw_conv_params *dw_conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q7_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
...
/**
 * @brief Optimized s8 depthwise convolution function for 3x3 kernel size with
 * some constraints on the input arguments(documented below). Refer
 * arm_depthwise_conv_s8() for function argument details.
 *
 * @return     The function returns one of the following
 *                <code>ARM_MATH_SIZE_MISMATCH</code> - Unsupported dimension of
 * tensors <code>ARM_MATH_ARGUMENT_ERROR</code> - Unsupported pad size along the
 * x axis <code>ARM_MATH_SUCCESS</code> - Successful operation
 *
 * @details
 *   - Supported framework : TensorFlow Lite Micro
 *   - The following constrains on the arguments apply
 ...
 *
 */
arm_status arm_depthwise_conv_3x3_s8(
    const cmsis_nn_context *ctx, const cmsis_nn_dw_conv_params *dw_conv_params,
    const cmsis_nn_per_channel_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q7_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
...
/**
 * @brief Optimized s8 depthwise convolution function with constraint that
 * in_channel equals out_channel. Refer arm_depthwise_conv_s8() for function
 * argument details.
 *
 * @return     The function returns one of the following
 *                <code>ARM_MATH_SIZE_MISMATCH</code> - input channel != output
 * channel or ch_mult != 1 <code>ARM_MATH_SUCCESS</code> - Successful operation
 *
 * @note       If number of channels is not a multiple of 4, upto 3 elements
 * outside the boundary will be read out for the following if MVE
 * optimizations(Arm Helium Technology) are used.
 *               - Output shift
 *               - Output multiplier
 *               - Output bias
 *               - kernel
 ...
 *    - Supported framework: TensorFlow Lite
 *    - The following constrains on the arguments apply
 *        -# Number of input channel equals number of output channels or ch_mult
 * equals 1
 *    - q7 is used as data type eventhough it is s8 data. It is done so to be
 * consistent with existing APIs.
 ...
 * Check the function definition file to see if an additional buffer is
 * required. Optional function {API}_get_buffer_size() provides the buffer size
 * if an additional buffer is required.
 * @param[in]      fc_params      Fully Connected layer parameters.
 *                                Range of fc_params->input_offset  : [-127,
 * 128] fc_params->filter_offset : 0 Range of fc_params->output_offset : [-128,
 * 127]
 * @param[in]      quant_params   Per-tensor quantization info.
 *                                It contains the multiplier and shift values to
 * be applied to the output tensor.
 * @param[in]      input_dims     Input (activation) tensor dimensions. Format:
 * [N, H, W, C_IN] Input dimension is taken as Nx(H * W * C_IN)
 * @param[in]      input_data     Input (activation) data pointer. Data type:
 * int8
 * @param[in]      filter_dims    Two dimensional filter dimensions. Format: [N,
 * C] N : accumulation depth and equals (H * W * C_IN) from input_dims C :
 * output depth and equals C_OUT in output_dims H & W : Not used
 * @param[in]      filter_data    Filter data pointer. Data type: int8
 * @param[in]      bias_dims      Bias tensor dimensions. Format: [C_OUT]
 *                                N, H, W : Not used
 * @param[in]      bias_data      Bias data pointer. Data type: int32
 ...
 * @details
 *    - Supported framework: TensorFlow Lite
 *    - q15 is used as data type eventhough it is s16 data. It is done so to be
 * consistent with existing APIs.
 */
arm_status arm_fully_connected_s16(
    const cmsis_nn_context *ctx, const cmsis_nn_fc_params *fc_params,
    const cmsis_nn_per_tensor_quant_params *quant_params,
    const cmsis_nn_dims *input_dims, const q15_t *input_data,
    const cmsis_nn_dims *filter_dims, const q7_t *filter_data,
...
q7_t *arm_nn_mat_mult_kernel_q7_q15(const q7_t *pA, const q15_t *pInBuffer,
                                    ...
                                    q7_t *pOut);
/**
 * @brief Matrix-multiplication function for convolution with per-channel
 * requantization.
 * @param[in]       input_a     pointer to operand A
 * @param[in]       input_b     pointer to operand B, always consists of 2
 * vectors.
 * @param[in]       output_ch   number of rows of A
 * @param[in]       out_shift  pointer to per output channel requantization
 * shift parameter.
 * @param[in]       out_mult   pointer to per output channel requantization
 * multiplier parameter.
 * @param[in]       out_offset      output tensor offset.
 * @param[in]       activation_min   minimum value to clamp the output to. Range
 * : int8
 * @param[in]       activation_max   maximum value to clamp the output to. Range
 * : int8
 ...
 *
 * @details   This function does the matrix multiplication of weight matrix for
 * all output channels with 2 columns from im2col and produces two
 * elements/output_channel. The outputs are clamped in the range provided by
 * activation min and max. Supported framework: TensorFlow Lite micro.
 */
q7_t *arm_nn_mat_mult_kernel_s8_s16(
    const q7_t *input_a, const q15_t *input_b, const uint16_t output_ch,
 ...
 *
 * @details  For arguments, refer arm_nn_mat_mult_kernel_s8_s16. The re-ordering
 * is a consequence of sign extension done by the SXTB16 command on input_b. The
 * outputs are clamped in the range provided by activation min and max.
 *   * @details
 *   - Supported framework : TensorFlow Lite Micro
 *   - The following constrains on the arguments apply
 ...
 * @param[in]       input_1_offset          offset for input 1. Range: Range:
 * -127 to 128
 * @param[in]       input_1_mult            multiplier for input 1
 * @param[in]       input_1_shift           shift for input 1
 * @param[in]       input_2_offset          offset for input 2. Range: Range:
 * -127 to 128
 ...
 */
arm_status arm_elementwise_add_s8(
    ...
    const int32_t left_shift, int8_t *output, const int32_t out_offset,
 ...
 */
arm_status arm_elementwise_mul_s8(
    const int8_t *input_1_vect, const int8_t *input_2_vect,
    const int32_t input_1_offset, const int32_t input_2_offset, int8_t *output,
 ...
 * @param[in]      output_dims    Output tensor dimensions. Format: [H, W,
 * C_OUT] Argument N is not used. C_OUT equals C_IN.
 * @param[in, out] output_data    Output data pointer. Data type: int8
 * @return                        The function returns
 *                                    <code>ARM_MATH_SUCCESS</code> - Successful
 * operation
 *
 * @details
 *    - Supported Framework: TensorFlow Lite
...
void arm_softmax_with_batch_q7(const q7_t *vec_in, const uint16_t nb_batches,
                               const uint16_t dim_vec, q7_t *p_out);
/**
...
void arm_softmax_s8(const int8_t *input, const int32_t num_rows,
...
void arm_softmax_u8(const uint8_t *input, const int32_t num_rows,
                    const int32_t row_size, const int32_t mult,
                    const int32_t shift, const int32_t diff_min,
                    uint8_t *output);
 ...
 *
 * @note The output is expected to be in a memory area that does not overlap
 * with the input's
 *
 */
void arm_reshape_s8(const int8_t *input, int8_t *output,
...
/**
 * @brief int8/uint8 concatenation function to be used for concatenating
 * N-tensors along the X axis This function should be called for each input
 * tensor to concatenate. The argument offset_x will be used to store the input
 * tensor in the correct position in the output tensor
 *
 *        i.e.    offset_x = 0
 *                for(i = 0 i < num_input_tensors; ++i)
 *                {
 *                    arm_concatenation_s8_x(&input[i], ..., &output, ..., ...,
 * offset_x) offset_x += input_x[i]
 *                }
 *
 ...
 *        Unless specified otherwise, arguments are mandatory.
 *
 * @note This function, data layout independent, can be used to concatenate
 * either int8 or uint8 tensors because it does not involve any arithmetic
 * operation
 *
 * @param[in]  input    Pointer to input tensor
 * @param[in]  input_x  Width of input tensor
 ...
 * @param[in]  output_x Width of output tensor
 * @param[in]  offset_x The offset (in number of elements) on the X axis to
 * start concatenating the input tensor It is user responsibility to provide the
 * correct value
 *
 ...
 *
 */
void arm_concatenation_s8_x(const int8_t *input, const uint16_t input_x,
                            const uint16_t input_y, const uint16_t input_z,
                            const uint16_t input_w, int8_t *output,
                            const uint16_t output_x, const uint32_t offset_x);

/**
 * @brief int8/uint8 concatenation function to be used for concatenating
 * N-tensors along the Y axis This function should be called for each input
 * tensor to concatenate. The argument offset_y will be used to store the input
 * tensor in the correct position in the output tensor
 *
 *        i.e.    offset_y = 0
 *                for(i = 0 i < num_input_tensors; ++i)
 *                {
 *                    arm_concatenation_s8_y(&input[i], ..., &output, ..., ...,
 * offset_y) offset_y += input_y[i]
 *                }
 *
 *        This function assumes that the output tensor has:
 ...
 * @param[out] output   Pointer to output tensor
 * @param[in]  output_y Height of output tensor
 * @param[in]  offset_y The offset on the Y axis to start concatenating the
 ...
 *
 */
void arm_concatenation_s8_y(const int8_t *input, const uint16_t input_x,
                            const uint16_t input_y, const uint16_t input_z,
                            const uint16_t input_w, int8_t *output,
                            const uint16_t output_y, const uint32_t offset_y);

/**
 * @brief int8/uint8 concatenation function to be used for concatenating
 * N-tensors along the Z axis This function should be called for each input
 * tensor to concatenate. The argument offset_z will be used to store the input
 * tensor in the correct position in the output tensor
 *
 *        i.e.    offset_z = 0
 *                for(i = 0 i < num_input_tensors; ++i)
 *                {
 *                    arm_concatenation_s8_z(&input[i], ..., &output, ..., ...,
 * offset_z) offset_z += input_z[i]
 *                }
 *
 *        This function assumes that the output tensor has:
 *        -# The same width of the input tensor
 ...
 * @param[in]  offset_z The offset on the Z axis to start concatenating the
 * input tensor It is user responsibility to provide the correct value
 *
 * <b> Input constraints</b>
 ...
 *
 */
void arm_concatenation_s8_z(const int8_t *input, const uint16_t input_x,
                            const uint16_t input_y, const uint16_t input_z,
                            const uint16_t input_w, int8_t *output,
                            const uint16_t output_z, const uint32_t offset_z);

/**
 * @brief int8/uint8 concatenation function to be used for concatenating
 * N-tensors along the W axis (Batch size) This function should be called for
 * each input tensor to concatenate. The argument offset_w will be used to store
 * the input tensor in the correct position in the output tensor
 *
 *        i.e.    offset_w = 0
 *                for(i = 0 i < num_input_tensors; ++i)
 *                {
 *                    arm_concatenation_s8_w(&input[i], ..., &output, ..., ...,
 * offset_w) offset_w += input_w[i]
 *                }
 *
 *        This function assumes that the output tensor has:
 *        -# The same width of the input tensor
 ...
 * @param[in]  input_w  Batch size in input tensor
 * @param[out] output   Pointer to output tensor
 * @param[in]  offset_w The offset on the W axis to start concatenating the
 * input tensor It is user responsibility to provide the correct value
 *
 */
void arm_concatenation_s8_w(const int8_t *input, const uint16_t input_x,
                            const uint16_t input_y, const uint16_t input_z,
                            const uint16_t input_w, int8_t *output,
...
arm_status arm_svdf_s8(
    ...
    const cmsis_nn_dims *input_dims, const q7_t *input_data,
    const cmsis_nn_dims *state_dims, q15_t *state_data,
    const cmsis_nn_dims *weights_feature_dims, const q7_t *weights_feature_data,
    const cmsis_nn_dims *weights_time_dims, const q15_t *weights_time_data,
