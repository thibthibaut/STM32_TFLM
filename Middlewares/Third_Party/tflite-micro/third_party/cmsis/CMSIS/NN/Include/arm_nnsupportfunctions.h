...
/**
 * @brief Converts the elements from a q7 vector to a q15 vector with an added
 * offset
 ...
 *
 */
void arm_q7_to_q15_with_offset(const q7_t *src, q15_t *dst, uint32_t block_size,
                               q15_t offset);

/**
 * @brief Converts the elements of the q7 vector to reordered q15 vector with an
 * added offset
 * @param[in]       src        pointer to the q7 input vector
 * @param[out]      dst        pointer to the q15 output vector
 * @param[in]       block_size length of the input vector
 ...
 * @return none.
 *
 * @details  This function does the q7 to q15 expansion with re-ordering of
 * bytes. Re-ordering is a consequence of the sign extension intrinsic(DSP
 * extension). The tail (i.e., last (N % 4) elements) retains its original
 * order.
 *
 */
void arm_q7_to_q15_reordered_with_offset(const q7_t *src, q15_t *dst,
                                         uint32_t block_size, q15_t offset);
 ...
 *
 */
void arm_nn_accumulate_q7_to_q15(q15_t *dst, const q7_t *src,
                                 uint32_t block_size);

/**
 * @brief Depthwise conv on an im2col buffer where the input channel equals
 * output channel.
 ...
 * @param[in]    out_shift  pointer to per output channel requantization shift
 * parameter.
 ...
 */
q7_t *arm_nn_depthwise_conv_s8_core(
    const q7_t *row, const q15_t *col, const uint16_t num_ch,
    const int32_t *out_shift, const int32_t *out_mult, const int32_t out_offset,
    const int32_t activation_min, const int32_t activation_max,
    const uint16_t kernel_size, const int32_t *const output_bias, q7_t *out);

/**
 * @brief General Matrix-multiplication function with per-channel
 * requantization.
 ...
 * @param[in]       output_shift  pointer to per output channel requantization
 * shift parameter.
 * @param[in]       output_mult   pointer to per output channel requantization
 * multiplier parameter.
 * @param[in]       out_offset    output tensor offset.
 ...
 */
q7_t *arm_nn_mat_mult_s8(const q7_t *input_row, const q7_t *input_col,
                         const uint16_t output_ch, const uint16_t col_batches,
                         const int32_t *output_shift,
                         const int32_t *output_mult, const int32_t out_offset,
                         const int32_t col_offset, const int32_t row_offset,
                         const int16_t out_activation_min,
                         const int16_t out_activation_max,
                         const uint16_t row_len, const int32_t *const bias,
                         q7_t *out);
/**
 * @brief Matrix-multiplication function for convolution with per-channel
 * requantization for 16 bits convolution.
 ...
 */
q15_t *arm_nn_mat_mult_kernel_s16(
    const q7_t *input_a, const q15_t *input_b, const int32_t output_ch,
    const int32_t *out_shift, const int32_t *out_mult,
    const int16_t activation_min, const int16_t activation_max,
    const int32_t num_col_a, const int64_t *const output_bias, q15_t *out_0);
/**
 * @brief General Matrix-multiplication without requantization for one row & one
 * column
 ...
 * @return     The function returns the multiply-accumulated result of the row
 * by column.
 *
 * @details Pseudo-code
 *      *output = 0
...
/**
 * @brief General Matrix-multiplication without requantization for four rows and
 * one column
 * @param[in]       row_elements  number of row elements
 * @param[in]       offset        offset between rows. Can be the same as
 * row_elements. For e.g, in a 1x1 conv scenario with stride as 1.
 * @param[in]       row_base      pointer to row operand
 * @param[in]       col_base      pointer to col operand
 * @param[out]      sum_col       pointer to store sum of column elements
 * @param[out]      output        pointer to store result(4 int32's) of
 * multiply-accumulate
 * @return     The function returns the multiply-accumulated result of the row
 * by column
 *
 ...
 *          sum_col += col_base[i]
 */
arm_status arm_nn_mat_mul_core_4x_s8(
    const int32_t row_elements, const int32_t offset, const int8_t *row_base,
    const int8_t *col_base, int32_t *const sum_col, int32_t *const output);

/**
 * @brief General Matrix-multiplication function with per-channel
 * requantization. This function assumes:
 *        - LHS input matrix NOT transposed (nt)
 *        - RHS input matrix transposed (t)
 *
 *  @note This operation also performs the broadcast bias addition before the
 * requantization
 *
 * @param[in]  lhs                Pointer to the LHS input matrix
 * @param[in]  rhs                Pointer to the RHS input matrix
 * @param[in]  bias               Pointer to the bias vector. The length of this
 * vector is equal to the number of output columns (or RHS input rows)
 * @param[out] dst                Pointer to the output matrix with "m" rows and
 * "n" columns
 * @param[in]  dst_multipliers    Pointer to the multipliers vector needed for
 * the per-channel requantization. The length of this vector is equal to the
 * number of output columns (or RHS input rows)
 * @param[in]  dst_shifts         Pointer to the shifts vector needed for the
 * per-channel requantization. The length of this vector is equal to the number
 * of output columns (or RHS input rows)
 ...
 * @param[in]  activation_min     Minimum value to clamp down the output. Range
 * : int8
 * @param[in]  activation_max     Maximum value to clamp up the output. Range :
 * int8
 *
...
arm_status arm_nn_mat_mult_nt_t_s8(
    const q7_t *lhs, const q7_t *rhs, const q31_t *bias, q7_t *dst,
 ...
 * @param[in]      lhs_offset      Offset to be added to the input values of the
 * left-hand side vector. Range: -127 to 128
 * @param[in]      rhs_offset      Not used
 * @param[in]      dst_offset      Offset to be added to the output values.
 * Range: -127 to 128
 * @param[in]      dst_multiplier  Output multiplier
 * @param[in]      dst_shift       Output shift
 * @param[in]      rhs_cols        Number of columns in the right-hand side
 * input matrix
 * @param[in]      rhs_rows        Number of rows in the right-hand side input
 * matrix
 * @param[in]      activation_min  Minimum value to clamp the output to. Range:
 * int8
 * @param[in]      activation_max  Maximum value to clamp the output to. Range:
 ...
 *
 */
arm_status arm_nn_vec_mat_mult_t_s8(
    const q7_t *lhs, const q7_t *rhs, const q31_t *bias, q7_t *dst,
 ...
 *
 */
arm_status arm_nn_vec_mat_mult_t_s16(
    const q15_t *lhs, const q7_t *rhs, const q63_t *bias, q15_t *dst,
    const int32_t dst_multiplier, const int32_t dst_shift,
 ...
 * left-hand side vector. Range: -127 to 128
 * @param[in]      rhs_offset      Not used
 * @param[in]      scatter_offset  Address offset for dst. First output is
 * stored at 'dst', the second at 'dst + scatter_offset' and so on.
 * @param[in]      dst_multiplier  Output multiplier
 * @param[in]      dst_shift       Output shift
 * @param[in]      rhs_cols        Number of columns in the right-hand side
 * input matrix
...
arm_status arm_nn_vec_mat_mult_t_svdf_s8(
    const q7_t *lhs, const q7_t *rhs, q15_t *dst, const int32_t lhs_offset,
    const int32_t rhs_offset, const int32_t scatter_offset,
    const int32_t dst_multiplier, const int32_t dst_shift,
    const int32_t rhs_cols, const int32_t rhs_rows,
    const int32_t activation_min, const int32_t activation_max);

/**
 * @brief Depthwise convolution of transposed rhs matrix with 4 lhs matrices. To
 * be used in padded cases where the padding is -lhs_offset(Range: int8).
 * Dimensions are the same for lhs and rhs.
 *
 * @param[in]      lhs             Input left-hand side matrix
 * @param[in]      rhs             Input right-hand side matrix (transposed)
 * @param[in]      lhs_offset      LHS matrix offset(input offset). Range: -127
 * to 128
 * @param[in]      num_ch          Number of channels in LHS/RHS
 * @param[in]      out_shift       Per channel output shift. Length of vector is
 * equal to number of channels
 * @param[in]      out_mult        Per channel output multiplier. Length of
 * vector is equal to number of channels
 * @param[in]      out_offset      Offset to be added to the output values.
 * Range: -127 to 128
 * @param[in]      activation_min  Minimum value to clamp the output to. Range:
 * int8
 * @param[in]      activation_max  Maximum value to clamp the output to. Range:
 * int8
 * @param[in]       row_x_col       (row_dimension * col_dimension) of LHS/RHS
 * matrix
 * @param[in]      output_bias     Per channel output bias. Length of vector is
 * equal to number of channels
 ...
 * outside the boundary will be read out for the following.
 *                  - Output shift
 *                  - Output multiplier
 *                  - Output bias
 *                  - rhs
 */
q7_t *arm_nn_depthwise_conv_nt_t_padded_s8(
    const q7_t *lhs, const q7_t *rhs, const int32_t lhs_offset,
    const uint16_t num_ch, const int32_t *out_shift, const int32_t *out_mult,
    const int32_t out_offset, const int32_t activation_min,
    const int32_t activation_max, const uint16_t row_x_col,
    const int32_t *const output_bias, q7_t *out);

/**
 * @brief Depthwise convolution of transposed rhs matrix with 4 lhs matrices. To
 * be used in non-padded cases. Dimensions are the same for lhs and rhs.
 *
 * @param[in]      lhs             Input left-hand side matrix
 * @param[in]      rhs             Input right-hand side matrix (transposed)
 * @param[in]      lhs_offset      LHS matrix offset(input offset). Range: -127
 ...
 *                  - rhs
 */
q7_t *arm_nn_depthwise_conv_nt_t_s8(
    const q7_t *lhs, const q7_t *rhs, const int32_t lhs_offset,
    const uint16_t num_ch, const int32_t *out_shift, const int32_t *out_mult,
    const int32_t out_offset, const int32_t activation_min,
    const int32_t activation_max, const uint16_t row_x_col,
 ...
 *
 */
__STATIC_FORCEINLINE void arm_memset_q7(q7_t *dst, const q7_t val,
                                        uint32_t block_size) {
...
__STATIC_FORCEINLINE const q7_t *read_and_pad(const q7_t *source, q31_t *out1,
                                              q31_t *out2) {
...
/**
 * @brief read and expand one q7 word into two q15 words with reordering and add
 * an offset
 */
__STATIC_FORCEINLINE const q7_t *read_and_pad_reordered_with_offset(
    const q7_t *source, q31_t *out1, q31_t *out2, q31_t offset) {
  q31_t inA = arm_nn_read_q7x4_ia(&source);
...
__STATIC_FORCEINLINE q31_t arm_nn_requantize(const q31_t val,
  ...
  return arm_nn_divide_by_power_of_two(
      arm_nn_doubling_high_mult_no_sat(val * (1 << LEFT_SHIFT(shift)),
                                       multiplier),
      RIGHT_SHIFT(shift));
}
 ...
 * @param[in]       val                 Value to be requantized
 * @param[in]       reduced_multiplier  Reduced multiplier from range
 * {NN_Q31_MIN + 1, Q32_MAX} to {Q16_MIN + 1, Q16_MAX}
 * @param[in]       shift               left or right shift for 'val *
 * multiplier'
 *
 * @return          Returns (val * multiplier)/(2 ^ shift)
 *
...
#if defined(ARM_MATH_MVEI)
/**
 * @brief           Vector saturating doubling high multiply returning high
 * half.
 ...
 *
 */
__STATIC_FORCEINLINE int32x4_t arm_doubling_high_mult_mve(const int32x4_t m1,
                                                          const q31_t m2) {
...
arm_divide_by_power_of_two_mve(const int32x4_t dividend, const q31_t exponent) {
 ...
 *
 */
__STATIC_FORCEINLINE int32x4_t arm_requantize_mve(const int32x4_t val,
                                                  const q31_t multiplier,
                                                  const q31_t shift) {
  return arm_divide_by_power_of_two_mve(
      arm_doubling_high_mult_mve(vshlq_s32(val, vdupq_n_s32(LEFT_SHIFT(shift))),
                                 multiplier),
      RIGHT_SHIFT(shift));
}
...
__STATIC_FORCEINLINE int32x4_t arm_divide_by_power_of_two_mve_32x4(
    const int32x4_t dividend, const int32x4_t exponent) {
...
__STATIC_FORCEINLINE int32x4_t arm_requantize_mve_32x4(
    const int32x4_t val, const int32x4_t multiplier, const int32x4_t shift) {
  ...
  return arm_divide_by_power_of_two_mve_32x4(
      arm_doubling_high_mult_mve_32x4(vshlq_s32(val, left_shift), multiplier),
      right_shift);
}
#endif

// @note The following functions are used only for softmax layer, scaled bits =
// 5 assumed
...
__STATIC_FORCEINLINE int32_t arm_nn_exp_on_negative_values(int32_t val) {
  ...
  int32_t result =
      1895147668 +
      MUL_SAT(1895147668, x + DIV_POW2(MUL_SAT(DIV_POW2(MUL_SAT(x2, x2), 2) +
                                                   MUL_SAT(x2, x),
                                               715827883) +
                                           x2,
                                       1));
...
__STATIC_FORCEINLINE q31_t arm_nn_mult_by_power_of_two(const int32_t val,
  ...
  result =
      SELECT_USING_MASK(MASK_IF_NON_ZERO(val < -thresh), NN_Q31_MIN, result);
  return result;
}

__STATIC_FORCEINLINE int32_t
arm_nn_one_over_one_plus_x_for_x_in_0_1(int32_t val) {
