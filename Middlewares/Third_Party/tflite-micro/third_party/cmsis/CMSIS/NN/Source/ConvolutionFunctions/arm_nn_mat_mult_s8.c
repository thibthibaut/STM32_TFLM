...
/*
 * s8 General matrix multiplication function with per-channel requantization for
 * upto 4 column batches.
 *
...
q7_t *arm_nn_mat_mult_s8(const q7_t *input_row, const q7_t *input_col,
                         ...
                         const int16_t activation_max, const uint16_t row_len,
                         const int32_t *const bias, q7_t *out) {
  ...
  if (col_batches == 4) {
    for (int i_out_ch = 0; i_out_ch < output_ch; i_out_ch++) {
      ...
      for (int i_row_loop = 0; i_row_loop < row_loop_cnt; i_row_loop++) {
        ...
        const int16x8_t offset =
            vdupq_m_n_s16(vuninitializedq_s16(), col_offset, p);
        row_len_tmp -= 8;
    ...
    for (int i_col_batch = (col_batches & ~0x3);
         i_col_batch < (col_batches & 0x3); i_col_batch++) {
      for (int i_out_ch = 0; i_out_ch < output_ch; i_out_ch++) {
        int32_t row_len_tmp = row_len;
        ...
        acc_0 = arm_nn_requantize(acc_0, output_mult[i_out_ch],
                                  output_shift[i_out_ch]);
        acc_0 += out_offset;
