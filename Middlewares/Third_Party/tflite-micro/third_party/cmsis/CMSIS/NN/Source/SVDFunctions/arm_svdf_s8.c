...
arm_status arm_svdf_s8(
  ...
  for (int i_batch = 0; i_batch < input_batches; i_batch++) {
    q15_t *res_ptr = state_data + (time_batches * i_batch * feature_batches) +
                     (time_batches - 1);
    const q7_t *weight = weights_feature_data;
    const q7_t *input = input_data + i_batch * input_height;

    arm_status res = arm_nn_vec_mat_mult_t_svdf_s8(
        input, weight, res_ptr, -zp_in, 0, time_batches, multiplier_in,
        shift_in, input_height, feature_batches, in_activation_min,
        in_activation_max);
...

  {
    ...
    for (int i_batch = 0; i_batch < input_batches; i_batch++) {
      ...
      for (int i_feature_batch = 0; i_feature_batch < feature_batches;
           i_feature_batch++) {
        *ptr_a = 0;
        int32_t sum = 0;
  ...
  for (int i = 0; i < input_batches * unit_count; i++) {
    output_data[i] = (q7_t)CLAMP(
        arm_nn_requantize(buffer_b[i], multiplier_out, shift_2) + zp_out,
