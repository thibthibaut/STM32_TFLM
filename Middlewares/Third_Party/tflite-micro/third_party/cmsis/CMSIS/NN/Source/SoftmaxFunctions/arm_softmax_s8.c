...
#define SHIFT_START (24)
  ...
  const int32x4_t val_mod_minus_quarter =
      vandq_s32(val, vdupq_n_s32((1 << SHIFT_START) - 1)) -
      vdupq_n_s32(1 << SHIFT_START);
  ...
  const int32x4_t op_1 =
      DIV_POW2_MVE(MUL_SAT_MVE(x2, x2), 2) + MUL_SAT_MVE(x2, x);
  const int32x4_t op_2 =
      x + DIV_POW2_MVE(MUL_SAT_MVE(op_1, vdupq_n_s32(715827883)) + x2, 1);
  int32x4_t result =
      vdupq_n_s32(1895147668) + MUL_SAT_MVE(vdupq_n_s32(1895147668), op_2);
...
#define SELECT_IF_NON_ZERO(x)                                                 \
  {                                                                           \
    ...
    result =                                                                  \
        SELECT_USING_MASK(mask, MUL_SAT_MVE(result, vdupq_n_s32(x)), result); \
  }
  ...
  for (int i_num_rows = 0; i_num_rows < num_rows; ++i_num_rows) {
    ...
    for (int i = 0; i < (row_size & 3); i++) {
      ...
      if (diff >= diff_min) {
        const int32_t res =
            DIV_POW2(
                MUL_SAT(shifted_scale, EXP_ON_NEG(MUL_SAT(diff * mask, mult))),
                bits_over_unit) -
            128;
        output[tail_idx + i] =
            (int8_t)CLAMP(res, (int32_t)ACT_MAX, (int32_t)ACT_MIN);
      } else {
