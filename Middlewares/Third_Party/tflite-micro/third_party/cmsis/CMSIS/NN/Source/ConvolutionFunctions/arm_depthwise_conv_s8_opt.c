...
/*
 * Optimized s8 depthwise convolution function with constraint that in_channel
 * equals out_channel
 *
...
arm_status arm_depthwise_conv_s8_opt(
  ...
  for (int i_out_y = 0, base_idx_y = -pad_y; i_out_y < output_y;
       base_idx_y += stride_y, i_out_y++) {
    for (int i_out_x = 0, base_idx_x = -pad_x; i_out_x < output_x;
         base_idx_x += stride_x, i_out_x++) {
      for (int i_ker_y = base_idx_y; i_ker_y < base_idx_y + kernel_y;
           i_ker_y++) {
        for (int i_ker_x = base_idx_x; i_ker_x < base_idx_x + kernel_x;
             i_ker_x++) {
          if (i_ker_y < 0 || i_ker_y >= input_y || i_ker_x < 0 ||
              i_ker_x >= input_x) {
            arm_memset_q7(lhs_buffer, (int8_t)-input_offset,
                          (uint32_t)input_ch);
            padded = 1;
          } else {
            arm_memcpy_q7(lhs_buffer,
                          input + (i_ker_y * input_x + i_ker_x) * input_ch,
                          (uint32_t)input_ch);
          }
          lhs_buffer += input_ch;
        }
      ...
      if (buffer_count == 4) {
        ...
        if (padded == 0) {
          out = arm_nn_depthwise_conv_nt_t_s8(
              lhs_buffer, kernel, input_offset, input_ch, output_shift,
              output_mult, output_offset, output_activation_min,
              output_activation_max, kernel_size, bias, out);
        } else {
          out = arm_nn_depthwise_conv_nt_t_padded_s8(
              lhs_buffer, kernel, input_offset, input_ch, output_shift,
              output_mult, output_offset, output_activation_min,
              output_activation_max, kernel_size, bias, out);
          padded = 0;
  ...
  for (int i_buf = 0; i_buf < buffer_count; i_buf++) {
    ...
    for (int i_loop_cnt = 0, offset = 0; i_loop_cnt < loop_count;
      ...
      const int8_t *col_0 =
          lhs_buffer + (kernel_size * input_ch * i_buf) + offset;
  ...
  for (int i_out_y = 0; i_out_y < output_y; i_out_y++) {
    ...
    for (int i_out_x = 0; i_out_x < output_x; i_out_x++) {
      ...
      /* Out of bounds is only considered for the y axis as it provides a
         contiguous zero'ing opportunity than along the x axis */
      ...
      int32_t index = 0;
      if (ker_y_start != 0) {
        memset(&col_buffer[index], 0,
               (kernel_x * input_ch) * ker_y_start * sizeof(q15_t));
        ...
        for (int i_ker_x = 0; i_ker_x < kernel_x; i_ker_x++) {
          ...
          if (idx_x < 0 || idx_x >= input_x) {
            ...
            arm_q7_to_q15_with_offset(
                (q7_t *)input + (idx_y * input_x + idx_x) * input_ch,
      ...
      if (diff != 0) {
        memset(&col_buffer[index], 0,
               (kernel_x * input_ch) * diff * sizeof(q15_t));
      }
      ...
      while (row_count) {
        ...
        while (col_count) {
          ...
          them in the right order to use in a SMLAD instruction . One run of
          this loop produces 4 partial outputs with 8 MACs. */
   ...
   * Cortex-M3 */
  return arm_depthwise_conv_s8(ctx, dw_conv_params, quant_params, input_dims,
                               input, filter_dims, kernel, bias_dims, bias,
                               output_dims, output);
